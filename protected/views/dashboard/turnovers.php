...
$this->pageTitle=Yii::app()->name . ' - Turnover Data Analytics and Statistics';
$this->breadcrumbs=array(
  'Dashboard'=>array('default'),
  'Analytics'=>array('turnovers'),
  'Turnovers'=>array('turnovers'),
);
?>
<div class="row">
  <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Turnover Statistics</h4>
                        <hr/>
                        <div class="loader" id="turnoversGrowthTableLoader"></div>
                        <div id="loadTurnoversGrowthTable" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Turnover Trend Graph</h4>
                        <hr/>
                        <div class="loader" id="turnoversGrowthGraphLoader"></div>
                        <div id="loadTurnoversGrowthGraph" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
...
function loadAnalytics(){
  ...
  loadTurnoversGrowthAnalytics(branch,staff,formattedStartDate,formattedEndDate);
  loadTurnoversGrowthAnalyticsGraph(branch,staff,formattedStartDate,formattedEndDate);
}
...
function loadTurnoversGrowthAnalytics(branch,staff,start_date,end_date){
  clearDivContent('loadTurnoversGrowthTable');
  showLoader('turnoversGrowthTableLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    dataType:"json",
    url: "<?=Yii::app()->createUrl('dashboard/loadTurnoversGrowthAnalytics');?>",
    data: dataString,
    success: function(response){
     hideLoader('turnoversGrowthTableLoader');
     var message = 'Turnover statistics fetched successfully ...';
     displayNotification(messagePanel,successType,message);
     $('#loadTurnoversGrowthTable').html(response.tabulation);
    }
  });
  return false;
}

function loadTurnoversGrowthAnalyticsGraph(branch,staff,start_date,end_date){
  clearDivContent('loadTurnoversGrowthGraph');
  showLoader('turnoversGrowthGraphLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('dashboard/loadTurnoversGrowthGraph');?>",
    data: dataString,
    success: function(graph){
     hideLoader('turnoversGrowthGraphLoader');
     $('#loadTurnoversGrowthGraph').html(graph);
    }
  });
  return false;
}
...
function initializeAnalytics(){
 var formattedStartDate  = formatDate(defaultStartDate);
 var formattedEndDate    = formatDate(defaultEndDate);
 loadTurnoversGrowthAnalytics(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
 loadTurnoversGrowthAnalyticsGraph(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
}
