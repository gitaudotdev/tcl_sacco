...
$this->pageTitle=Yii::app()->name . ' - Customer Growth Data Analytics and Statistics';
$this->breadcrumbs=array(
  'Dashboard'=>array('default'),
  'Analytics'=>array('customerGrowth'),
  'Customer_Growth'=>array('customerGrowth'),
);
?>
<div class="row">
  <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Customer Growth</h4>
                        <hr/>
                        <div class="loader" id="customerGrowthTableLoader"></div>
                        <div id="loadCustomerGrowthTable" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Growth Trend Graph</h4>
                        <hr/>
                        <div class="loader" id="customerGrowthGraphLoader"></div>
                        <div id="loadCustomerGrowthGraph" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
...
function loadAnalytics(){
  ...
  loadCustomerGrowthAnalytics(branch,staff,formattedStartDate,formattedEndDate);
 loadCustomerGrowthAnalyticsGraph(branch,staff,formattedStartDate,formattedEndDate);
}
...
function loadCustomerGrowthAnalytics(branch,staff,start_date,end_date){
  clearDivContent('loadCustomerGrowthTable');
  showLoader('customerGrowthTableLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    dataType:"json",
    url: "<?=Yii::app()->createUrl('dashboard/loadCustomerGrowthAnalytics');?>",
    data: dataString,
    success: function(response){
     hideLoader('customerGrowthTableLoader');
     var message = 'Customer growth statistics fetched successfully ...';
     displayNotification(messagePanel,successType,message);
     $('#loadCustomerGrowthTable').html(response.tabulation);
    }
  });
  return false;
}

function loadCustomerGrowthAnalyticsGraph(branch,staff,start_date,end_date){
  clearDivContent('loadCustomerGrowthGraph');
  showLoader('customerGrowthGraphLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('dashboard/loadCustomerGrowthGraph');?>",
    data: dataString,
    success: function(graph){
     hideLoader('customerGrowthGraphLoader');
     $('#loadCustomerGrowthGraph').html(graph);
    }
  });
  return false;
}
...
function initializeAnalytics(){
 var formattedStartDate  = formatDate(defaultStartDate);
 var formattedEndDate    = formatDate(defaultEndDate);
 loadCustomerGrowthAnalytics(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
 loadCustomerGrowthAnalyticsGraph(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
}
