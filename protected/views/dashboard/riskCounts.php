...
$this->pageTitle=Yii::app()->name . ' - Total Loan Accounts Risk Classification Data Analytics and Statistics';
$this->breadcrumbs=array(
  'Dashboard'=>array('default'),
  'Analytics'=>array('riskCounts'),
  'Risk_Classification'=>array('riskCounts'),
);
?>
<div class="row">
  <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Total Loan Accounts Risk Classification Statistics</h4>
                        <hr/>
                        <div class="loader" id="riskCountsGrowthTableLoader"></div>
                        <div id="loadRiskCountsGrowthTable" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Total Loan Accounts Risk Classification</h4>
                        <hr/>
                        <div class="loader" id="riskCountsGrowthGraphLoader"></div>
                        <div id="loadRiskCountsGrowthGraph" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
...
var defaultBranch  = userLevel  === '0' ? 0 : "<?=Yii::app()->user->user_branch;?>";
...
function loadAnalytics(){
  ...
  loadRiskCountsGrowthAnalytics(branch,staff,formattedStartDate,formattedEndDate);
  loadRiskCountsGrowthAnalyticsGraph(branch,staff,formattedStartDate,formattedEndDate);
}
...
function loadRiskCountsGrowthAnalytics(branch,staff,start_date,end_date){
  clearDivContent('loadRiskCountsGrowthTable');
  showLoader('riskCountsGrowthTableLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    dataType:"json",
    url: "<?=Yii::app()->createUrl('dashboard/loadRiskCountsGrowthAnalytics');?>",
    data: dataString,
    success: function(response){
     hideLoader('riskCountsGrowthTableLoader');
     var message = 'Total loans risk classification statistics fetched successfully ...';
     displayNotification(messagePanel,successType,message);
     $('#loadRiskCountsGrowthTable').html(response.tabulation);
    }
  });
  return false;
}

function loadRiskCountsGrowthAnalyticsGraph(branch,staff,start_date,end_date){
  clearDivContent('loadRiskCountsGrowthGraph');
  showLoader('riskCountsGrowthGraphLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('dashboard/loadRiskCountsGrowthGraph');?>",
    data: dataString,
    success: function(graph){
     hideLoader('riskCountsGrowthGraphLoader');
     $('#loadRiskCountsGrowthGraph').html(graph);
    }
  });
  return false;
}
...
function initializeAnalytics(){
 var formattedStartDate  = formatDate(defaultStartDate);
 var formattedEndDate    = formatDate(defaultEndDate);
 loadRiskCountsGrowthAnalytics(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
 loadRiskCountsGrowthAnalyticsGraph(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
}
