<?php
$this->pageTitle=Yii::app()->name . ' - Microfinance : Arrears Report';
$this->breadcrumbs=array(
	'ArrearsReport'=>array('reports/feesReport'),
);
?>
<style type="text/css">
 #date_error{
  ...
  <div class="col-md-12 col-lg-12 col-sm-12">
    <div class="card">
        <div class="card-header col-md-12 col-lg-12 col-sm-12">
            <h5 class="title">Arrears Report</h5>
            <hr>
        </div>
        <div class="card-body">
        	<div class="col-md-12 col-lg-12 col-sm-12">
              ...
              <div class="row">
                    ...
                    <div class="col-md-4 col-lg-4 col-sm-12">
                        <div class="form-group" style="margin-top: -5%">
                          <button type="button" class="btn btn-primary btn-block" id="generate_cmd"> <i class="now-ui-icons ui-1_zoom-bold"></i> View Arrears Report</button>
                        </div>
                  </div>
              </div>
              <br>
  ...
  $(function(){
    ...
    $("#generate_cmd").click(function(){
      ...
      if(startDate == '' && endDate == ''){
        ...
        var startingDate=new Date(date.getFullYear()- 25,date.getMonth(), 1);
      ...
      LoadArrearsReport(startDate,endDate,branch,staff,borrower);
    });
  });
...
function LoadArrearsReport(startDate,endDate,branch,staff,borrower){
    $('.error').hide();
    $('.loadingData').show();
    var dataString = 'start_date='+ startDate + '&end_date=' + endDate+ '&branch=' + branch+ '&staff=' + staff + '&borrower=' + borrower;
    $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('reports/filterArrearsReport');?>",
    data: dataString,
    success: function(response){
      $('.loadingData').hide();
...
function initFiltration(){
  ...
  var startDate=new Date(date.getFullYear() - 25, date.getMonth(), 1);
  var formattedStartDate=formatDate(startDate);
  LoadArrearsReport(formattedStartDate,formattedEndDate,branch,staff,borrower);
}
</script>