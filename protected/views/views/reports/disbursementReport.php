<?php
$this->pageTitle=Yii::app()->name . ' - Microfinance : Disbursement Report';
$this->breadcrumbs=array(
	'DisbursementReport'=>array('reports/disbursementReport'),
);
/**Flash Messages**/
$successType = 'success';
  ...
  <div class="col-md-12 col-lg-12 col-sm-12">
    <div class="card">
        <div class="card-header">
            <div class="col-md-12 col-lg-12 col-sm-12">
              <h5 class="title">Disbursement Report</h5>
              <hr>
          </div>
        </div>
        <div class="card-body">
              ...
              <div class="row">
                    ...
                    <div class="col-md-4 col-lg-4 col-sm-12">
                      <div class="form-group" style="margin-top: -5%">
                        <button type="button" class="btn btn-primary btn-block" id="disbursement_cmd"> <i class="now-ui-icons ui-1_zoom-bold"></i> 
                        View Disbursement Report</button>
                      </div>
                  </div>
              </div>
              <br>
          ...
          <div class="col-md-12 col-lg-12 col-sm-12">
            ...
            <div id="LoadDisbursementReport"></div>
          </div>
        </div>
     </div>
  </div>
...
$(function(){
  ...
  $("#disbursement_cmd").click(function(){
    ...
    FilterDisbursementReport(startDate,endDate,branch,staff,borrower);
  });
});
...
function FilterDisbursementReport(startDate,endDate,branch,staff,borrower){
    $('.loadingData').show();
    var dataString='start_date='+startDate+'&end_date='+endDate+'&branch='+branch+'&staff='+staff
    +'&borrower='+ borrower;
    $.ajax({
      type:"POST",
      url: "<?=Yii::app()->createUrl('reports/filterDisbursementReport');?>",
      data: dataString,
      success: function(response){
        $('.loadingData').hide();
        if(response === 'NOT FOUND'){
          $("#overall").html("<div class='col-md-12 col-lg-12 col-sm-12' style='padding:10px 10px 10px 10px !important;'><p style='border-bottom: 3px dotted #ddd;font-size:1.39em;color:#00933b;'><strong style='margin-left:20% !important;'>NO LOANS FOUND</strong></p><br><p style='color:#f90101;font-size:1.30em;'>*** NO LOANS WERE FOUND BY THE SPECIFIED FILTERS. ****</p></div>");
        }else{
          $('#LoadDisbursementReport').html(response).show().fadeIn('slow');
        }
      }
    });
}
...
function initFiltration(){
  ...
  FilterDisbursementReport(formattedStartDate,formattedEndDate,branch,staff,borrower);
}
</script>