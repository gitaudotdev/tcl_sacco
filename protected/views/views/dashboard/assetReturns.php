...
$this->pageTitle=Yii::app()->name . ' - Return on Assets Data Analytics and Statistics';
$this->breadcrumbs=array(
  'Dashboard'=>array('default'),
  'Analytics'=>array('assetReturns'),
  'Return_on_Assets'=>array('assetReturns'),
);
?>
<div class="row">
  <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Return on Assets Statistics</h4>
                        <hr/>
                        <div class="loader" id="assetReturnsGrowthTableLoader"></div>
                        <div id="loadAssetReturnsGrowthTable" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Return on Assets Graph</h4>
                        <hr/>
                        <div class="loader" id="assetReturnsGrowthGraphLoader"></div>
                        <div id="loadAssetReturnsGrowthGraph" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
...
function loadAnalytics(){
  ...
  loadAssetReturnsGrowthAnalytics(branch,staff,formattedStartDate,formattedEndDate);
  loadAssetReturnsGrowthAnalyticsGraph(branch,staff,formattedStartDate,formattedEndDate);
}
...
function loadAssetReturnsGrowthAnalytics(branch,staff,start_date,end_date){
  clearDivContent('loadAssetReturnsGrowthTable');
  showLoader('assetReturnsGrowthTableLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    dataType:"json",
    url: "<?=Yii::app()->createUrl('dashboard/loadAssetReturnsGrowthAnalytics');?>",
    data: dataString,
    success: function(response){
     hideLoader('assetReturnsGrowthTableLoader');
     var message = 'Return on assets statistics fetched successfully ...';
     displayNotification(messagePanel,successType,message);
     $('#loadAssetReturnsGrowthTable').html(response.tabulation);
    }
  });
  return false;
}

function loadAssetReturnsGrowthAnalyticsGraph(branch,staff,start_date,end_date){
  clearDivContent('loadAssetReturnsGrowthGraph');
  showLoader('assetReturnsGrowthGraphLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('dashboard/loadAssetReturnsGrowthGraph');?>",
    data: dataString,
    success: function(graph){
     hideLoader('assetReturnsGrowthGraphLoader');
     $('#loadAssetReturnsGrowthGraph').html(graph);
    }
  });
  return false;
}
...
function initializeAnalytics(){
 var formattedStartDate  = formatDate(defaultStartDate);
 var formattedEndDate    = formatDate(defaultEndDate);
 loadAssetReturnsGrowthAnalytics(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
 loadAssetReturnsGrowthAnalyticsGraph(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
}
