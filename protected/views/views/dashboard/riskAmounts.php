...
$this->pageTitle=Yii::app()->name . ' - Amount Disbursed Risk Classification Data Analytics and Statistics';
$this->breadcrumbs=array(
  'Dashboard'=>array('default'),
  'Analytics'=>array('riskAmounts'),
  'Risk_Classification_Amount_Disbursed'=>array('riskAmounts'),
);
?>
<div class="row">
  <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Amount Disbursed Risk Classification Statistics</h4>
                        <hr/>
                        <div class="loader" id="riskAmountsGrowthTableLoader"></div>
                        <div id="loadRiskAmountsGrowthTable" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card card-stats card-raised-modified">
            <div class="card-body">
                <div class="row">
                  <div class="col-md-12 col-lg-12 col-sm-12">
                        <h4 class="info-title">Amount Disbursed Risk Classification Graph</h4>
                        <hr/>
                        <div class="loader" id="riskAmountsGrowthGraphLoader"></div>
                        <div id="loadRiskAmountsGrowthGraph" class="loaded-content"></div>
                    </div>
                </div>
            </div>
        </div>
...
function loadAnalytics(){
  ...
  loadRiskAmountsGrowthAnalytics(branch,staff,formattedStartDate,formattedEndDate);
  loadRiskAmountsGrowthAnalyticsGraph(branch,staff,formattedStartDate,formattedEndDate);
}
...
function loadRiskAmountsGrowthAnalytics(branch,staff,start_date,end_date){
  clearDivContent('loadRiskAmountsGrowthTable');
  showLoader('riskAmountsGrowthTableLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    dataType:"json",
    url: "<?=Yii::app()->createUrl('dashboard/loadRiskAmountsGrowthAnalytics');?>",
    data: dataString,
    success: function(response){
     hideLoader('riskAmountsGrowthTableLoader');
     var message = 'Amount disbursed risk classification statistics fetched successfully ...';
     displayNotification(messagePanel,successType,message);
     $('#loadRiskAmountsGrowthTable').html(response.tabulation);
    }
  });
  return false;
}

function loadRiskAmountsGrowthAnalyticsGraph(branch,staff,start_date,end_date){
  clearDivContent('loadRiskAmountsGrowthGraph');
  showLoader('riskAmountsGrowthGraphLoader',content);
  var dataString ='branch='+branch+'&staff='+staff+'&start_date='+start_date+'&end_date='+end_date;
  $.ajax({
    type:"POST",
    url: "<?=Yii::app()->createUrl('dashboard/loadRiskAmountsGrowthGraph');?>",
    data: dataString,
    success: function(graph){
     hideLoader('riskAmountsGrowthGraphLoader');
     $('#loadRiskAmountsGrowthGraph').html(graph);
    }
  });
  return false;
}
...
function initializeAnalytics(){
 var formattedStartDate  = formatDate(defaultStartDate);
 var formattedEndDate    = formatDate(defaultEndDate);
 loadRiskAmountsGrowthAnalytics(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
 loadRiskAmountsGrowthAnalyticsGraph(defaultBranch,defaultStaff,formattedStartDate,formattedEndDate);
}
