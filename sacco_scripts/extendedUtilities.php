...
function getSaleOrderIdByOrderNumber($conn,$clientAccount){
    $stkQuery = "SELECT id FROM orders WHERE orderNumber='$clientAccount'";
	$result   = $conn->query($stkQuery);
	if(!empty($result)) {
		while($row = $result->fetch_assoc()){
			$orderId = $row['id'];
...
function getDateDifference($repaymentDate,$today){
	...
	$differenceDays = (int)$difference->format('%d');
...
function InitiatePayment($conn,$transaction_id,$clientAccount,$phoneNumber,$amount,$description,$firstname,$lastname,$date){
	...
	$branchAirtime = getBranchByAirtimeAccount($conn,$clientAccount);
	if($branchAirtime != 0){
		foreach($branchAirtime AS $branch){
			$branchId  = $branch["branch_id"];   
			$profileId = $branch["managerId"];
			$branchAirtimeAccount = $branch["airtimeAccount"];
		}
		$standard  = 0;
		$premium   = 0;
		$direction = "INCOMING";
		$category  = "M-PESA AIRTIME DEPOSIT";
		$aurtimeDesc = $category.':'.$branchAirtimeAccount;
		$airtimeReceipt = $transaction_id;
		createAirtimeTransactions($conn,$branchId,$profileId,$amount,$standard,$premium,$direction,$category,$aurtimeDesc,$airtimeReceipt);
	}

	$branchFunds = getBranchByFundsAccount($conn,$clientAccount);
	if($branchFunds != 0){
		foreach($branchFunds AS $branch){
			$branchId  = $branch["branch_id"];
			$profileId = $branch["managerId"];
			$branchFundsAccount = $branch["fundsAccount"];
		}
		$standard  = 0;
		$charge    = 0;
		$direction = "INCOMING";
		$category  = "M-PESA DEPOSIT FLOAT";
		$payment_description = $category.':'.$branchFundsAccount;
		$payReceipt = $transaction_id;
		createFundsTransactions($conn,$branchId,$profileId,$amount,$direction,$category,$payment_description,$standard,$charge,$payReceipt,$branchFundsAccount,$phoneNumber);
	}

	// check stk record using client account
	$stkRecord = getSTKPushRecordByAccount($conn,$clientAccount);
	$orderID   = getSaleOrderIdByOrderNumber($conn,$clientAccount);
	if($stkRecord != 0 && $orderID > 0){
		foreach($stkRecord AS $record){
			$branchId  = $record["branchId"];
			$profileId = $record["profileId"];
			$resultCode = $record["resultCode"];
			$transactionType = $record["transactionType"];
		}
		if($transactionType == "PRODUCT_SALES"){
			switch($resultCode){
				case '0':
				$paid = payDebt($conn,$orderID);
				if($paid == 1000){
					$standard  = 0;
					$charge    = 0;
					$direction = "INCOMING";
					$category  = "M-PESA PRODUCT SALE";
					$payment_description = $category.':'.$clientAccount;
					$payReceipt = $transaction_id;
					createFundsTransactions($conn,$branchId,$profileId,$amount,$direction,$category,$payment_description,$standard,$charge,$payReceipt,$clientAccount,$phoneNumber);
				}
				break;

				default:
				cancelDebt($conn,$orderId);
				break;
			}
		}
	}
	...
	if($userID != 0  && $branchFunds == 0 && $branchAirtime == 0 && $stkRecord == 0){
		$loanaccountID = getLoanAccount($conn,$userID);
		$userArray     = getAccountHolder($conn,$userID);
		if($loanaccountID != 0){
			createLoanRepayment($conn,$loanaccountID,$amount,$firstname,$clientAccount,$phoneNumber,$userArray,$transaction_id);
		}else{
...
function payDebt($conn,$orderId){
	$cashSaleDate  = date('Y-m-d H:i:s');
	$paymentQuery  = "UPDATE orders SET saleType='CASH', createdAt='$cashSaleDate' WHERE id=$orderId";
	if($conn->query($paymentQuery)){
		$updateOrderQuery = "UPDATE orderItems SET saleType='CASH', createdAt='$cashSaleDate' WHERE orderId=$orderId";
...
function cancelDebt($conn,$orderId){
	$voidQuery = "UPDATE orders SET isVoid='1' WHERE id=$orderId";
	if($conn->query($voidQuery)){
		cancelOrderItems($conn,$orderId);
	}
}

function cancelOrderItems($conn,$orderId){
	if(voidOrderItems($conn,$orderId) == 1000){
		$items = getSaleOrderItems($conn,$orderId);
		if($items != 0){
			foreach($items AS $item){
				$itemId    = $item['itemId'];
				$units     = $item['units'];
				$direction = "INCOMING";
				reinstateOrderItemQuantity($conn,$itemId,$units,$direction);
			}
		}
	}
}

function getSaleOrderItems($conn,$orderId){
	$orderItemsQuery = "SELECT * FROM orderItems WHERE orderId=$orderId";
	$result   = $conn->query($orderItemsQuery);
	if(!empty($result)) {
		$itemArray = [];
		while($row = $result->fetch_assoc()){
			$itemArray[] = $row;
...
function voidOrderItems($conn,$orderId){
	$voidQuery = "UPDATE orderItems SET isVoid='1' WHERE orderId=$orderId";
	return $conn->query($voidQuery) ? 1000 : 1003;
}

function reinstateOrderItemQuantity($conn,$itemId,$units,$direction){
	$dateStocked  = date('Y-m-d H:i:s');
	$restockQuery = "INSERT INTO itemsRestock(itemId,units,direction,restockedBy,restockedAt)
	VALUES('$itemId',$units,'$direction',1,'$dateStocked')";
	if($conn->query($restockQuery)){
		echo "It is done";
	}
}
...
function processPayroll($userID,$branchID,$salesCommision,$collectionsCommision,$totalLoan,$grossSalary,$netSalary,$payMonth,$payYear,$conn){
	...
	switch(restrictDuplicatePayroll($userID,$payMonth,$payYear,$conn)){
		...
		if($userArray !=0 ){
			...
			switch(isFundsBalanceSufficient($conn,$branchId,$netSalary,TRUE)){
				case 1000:
					...
					switch($payStatus){
						...
						if($transactionID > 0){
							switch(recordPayrollLog($userID,$branchID,$salesCommision,$collectionsCommision,$totalLoan,$grossSalary,$netSalary,$payMonth,$payYear,$transactionID,$conn)){
								...
								// AUTOMATED PAYROLL - INCOMING PAYMENT
								$standard  = 0;
								$charge    = 0;
								$direction = "INCOMING";
								$category  = "M-PESA SAVINGS DEPOSIT";
								$payment_description = $category.':'.$transaction_id;
								$payReceipt = $transaction_id;
								createFundsTransactions($conn,$branchId,$profileId,$amount,$direction,$category,$payment_description,$standard,$charge,$payReceipt,$rawClientAccount,$phoneNumber);
								$message           = "$branchName has processed and disbursed your ".$payrollPeriod." salary. Thank you!";
								$textMessage       = "Dear ".strtoupper($firstName).",".$message;
				...
				case 1001:
				echo "BRANCH HAS INSUFFICIENT FUNDS";
				break;
			}
		}else{
...
function checkOffLoan($userID,$totalLoan,$conn){
	...
	if($userArray !=0 ){
		...
		if($phoneNumber != 0){
			if($loanaccountID > 0){
				$transaction_id = "SALCHK".determineReceiptNumber($conn,"funds");
				createLoanRepayment($conn,$loanaccountID,$totalLoan,$firstname,$clientAccount,$phoneNumber,$userArray,$transaction_id);
			}
		}else{
...
function createSavingTransaction($conn,$savingAccountID,$amount,$firstname,$rawClientAccount,$phoneNumber,$userID,$transaction_id){
	...
	if($amount > 0){
		...
		if($conn->query($saveQuery)){
			// SAVINGS DEPOSIT - INCOMING PAYMENT
			$standard  = 0;
			$charge    = 0;
			$direction = "INCOMING";
			$category  = "M-PESA SAVINGS DEPOSIT";
			...
			$payReceipt = $transaction_id;
			createFundsTransactions($conn,$branchId,$profileId,$amount,$direction,$category,$payment_description,$standard,$charge,$payReceipt,$rawClientAccount,$phoneNumber);
			$message = "Thank you ".$firstname.", your savings of KES ".$amount." to account ".$rawClientAccount." is received. Your account is now updated.";
...
function createLoanRepayment($conn,$loanaccount_id,$amount,$firstname,$clientAccount,$phoneNumber,$userArray,$transaction_id){
  	$transactionID = createLoanTransaction($conn,$loanaccount_id,$amount,$clientAccount,$phoneNumber,$userArray,$transaction_id);
...
function createLoanTransaction($conn,$loanaccount_id,$amount,$clientAccount,$phoneNumber,$userArray,$transaction_id){
...
function createRepaymentBreakDown($conn,$accountID,$amountPaid,$transactionID,$firstname,$clientAccount,$phoneNumber,$userArray){
	...
	if($userArray !=0 ){
		foreach($userArray AS $user){
			...
			$accountFirstName   = strtoupper($user['firstName']);
		}
		$phones = getProfileContactByTypeOrderDesc($conn,$userID,'PHONE');
		foreach($phones AS $phone){
			$pNumber = $phone['contactValue'];
		...
		$branchArray = getRespectiveBranch($userBranch,$conn);
		if($branchArray != 0){
			foreach($branchArray AS $branch){
				$branchName  = strtoupper($branch['name']);
			}
	...
	if($conn->query($payQuery)){
		$message  = "Thank you ".strtoupper($firstname).", $branchName has received your payment of KES ".asMoney($amountPaid)." to account ". $clientAccount.". Your account is now updated. Thank you!";
	    sendNotification($message,$phoneNumber,$conn,'14',$userID);
		$amountBalance  = number_format(getLoanBalance($accountID,$conn),2);
		$balanceMessage = "Dear ".$accountFirstName.", your $branchName account is now updated. New balance is Kshs ".$amountBalance."/-. For queries please contact your Account Manager. Thank you!";
...
function createStrayRepaymentRecord($conn,$transaction_id,$clientAccount,$amount,$description,$firstname,$lastname,$date,$phoneNumber,$userID){
	...
	if($conn->query($straySQL)){
		$message = "Thank you ".$firstname.", your payment of KES ".asMoney($amount)." to account ".$clientAccount." is received. Please contact your manager for more info";
		sendNotification($message,$phoneNumber,$conn,'14',$userID);
...
/***
 * 
 * BRANCH FUNDS AND AIRTIME 
 * 
 * TRANSACTIONS OPERATIONS
 * 
 */
function getBranchByAirtimeAccount($conn,$airtimeAccount){
	$branchQuery = "SELECT * FROM branch WHERE airtimeAccount='$airtimeAccount' AND is_merged='0'";
	$branch = $conn->query($branchQuery);
...
function getBranchByFundsAccount($conn,$fundsAccount){
	$branchQuery = "SELECT * FROM branch WHERE fundsAccount='$fundsAccount' AND is_merged='0'";
	$branch = $conn->query($branchQuery);
	if($branch->num_rows > 0){
		while($row = $branch->fetch_assoc()){
			$branchArray[] = $row;
...
function determineAccountNumber($conn,$accountType,$managerID){
	$accountPrefix = $accountType === "airtime" ? "ATM" : "FDS";
	$profileArray = getAccountHolder($conn,$managerID);
	if($profileArray !=0 ){
		foreach($profileArray AS $profile){
			$idNumber  = $profile['idNumber'];
		}
		$accountNumber = $accountPrefix.$idNumber;
	}else{
...
function getFundsBalance($conn,$branchId){
	$incomingFunds   = getTotalIncomingFunds($conn,$branchId);
	$outgoingFunds   = getTotalOutgoingFunds($conn,$branchId);
	$outgoingCharges = getTotalOutgoingFundsCharges($conn,$branchId);
	$balance = $incomingFunds - ($outgoingFunds + $outgoingCharges);
	return $balance <= 0 ? 0 : $balance;
}

function getTotalIncomingFunds($conn,$branchId){
	$incomingQuery = "SELECT COALESCE(SUM(amount),0) AS amount FROM fundsTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('INCOMING')";
	$incoming      = $conn->query($incomingQuery);
...
function getTotalOutgoingFunds($conn,$branchId){
	$outgoingQuery = "SELECT COALESCE(SUM(amount),0) AS amount FROM fundsTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('OUTGOING')";
	$outgoing      = $conn->query($outgoingQuery);
	if($outgoing->num_rows > 0){
		while($row = $outgoing->fetch_assoc()){
			$outgoingAmount = $row['amount'];
		}
	}else{
		$outgoingAmount = 0;
	}
...
function getTotalOutgoingFundsCharges($conn,$branchId){
	$outgingChargeQuery = "SELECT SUM(charge) AS charge FROM fundsTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('OUTGOING')";
	$outgoing      = $conn->query($outgingChargeQuery);
	if($outgoing->num_rows > 0){
		while($row = $outgoing->fetch_assoc()){
			$outgoingChargeAmount = $row['charge'];
		}
	}else{
		$outgoingChargeAmount = 0;
	}
	return $outgoingChargeAmount;	
}

function getAirtimeBalance($conn,$branchId){
	$incomingAirtime = getTotalIncomingAirtime($conn,$branchId);
	$outgoingAirtime = getTotalOutgoingAirtime($conn,$branchId);
	$outgoingCharges = getTotalOutgoingAirtimeCharges($conn,$branchId);
	$airtimeBalance  = $incomingAirtime - ($outgoingAirtime + $outgoingCharges);
	return $airtimeBalance <= 0 ? 0 : $airtimeBalance;
}

function getTotalIncomingAirtime($conn,$branchId){
	$incomingQuery = "SELECT SUM(amount) AS amount FROM airtimeTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('INCOMING')";
	$incoming      = $conn->query($incomingQuery);
	if($incoming->num_rows > 0){
		while($row = $incoming->fetch_assoc()){
...
function getTotalOutgoingAirtime($conn,$branchId){
	$outgoingQuery = "SELECT SUM(amount) AS amount FROM airtimeTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('OUTGOING')";
	$outgoing      = $conn->query($outgoingQuery);
	if($outgoing->num_rows > 0){
		while($row = $outgoing->fetch_assoc()){
...
function getTotalOutgoingAirtimeCharges($conn,$branchId){
	$outgoingChargesQuery = "SELECT SUM(premium) AS premium FROM airtimeTransactions WHERE
	branchId=$branchId AND isVoid IN('0') AND direction IN('OUTGOING')";
	$outgoing      = $conn->query($outgoingChargesQuery);
	if($outgoing->num_rows > 0){
		while($row = $outgoing->fetch_assoc()){
...
function determineReceiptNumber($conn,$context){
	$prefix = $context === "airtime" ? "ATM" : "FDS";
	$nextcounter = $context === "airtime" ? getNextAirtimeTransaction($conn) : getNextFundsTransaction($conn);
	return $prefix.date("Ymd").$nextcounter;
}

function getNextAirtimeTransaction($conn){
	$airtimeQuery = "SELECT * FROM airtimeTransactions ORDER BY id DESC LIMIT 1";
	$airtimes     = $conn->query($airtimeQuery);
	$current = 0;
	if(!empty($airtimes)) {
		while($row = $airtimes->fetch_assoc()){
			$airtimeId = $row['id'];
		}
		$current = $airtimeId;
	}else{
	...
	return padNumber($current+1);
}

function getNextFundsTransaction($conn){
	$fundsQuery = "SELECT * FROM fundsTransactions ORDER BY id DESC LIMIT 1";
	$funds   = $conn->query($fundsQuery);
	$current = 0;
	if(!empty($funds)) {
...
function createFundsTransactions($conn,$branchId,$profileId,$amount,$direction,$category,$description,$standard,
$charge,$receiptNumber,$accountNumber,$phoneNumber){
	$dateCreatedAt = date('Y-m-d H:i:s');
	$fundQuery = "INSERT INTO fundsTransactions (branchId,profileId,amount,charge,standard,accountNumber,phoneNumber,direction,category,description,receiptNumber,createdAt)
	VALUES('$branchId','$profileId','$amount','$charge','$standard','$accountNumber','$phoneNumber','$direction','$category','$description','$receiptNumber','$dateCreatedAt')";
	if($conn->query($fundQuery)){
		/**
		 * LOG ACTION
		 * 
		 * SEND SMS
		 */
		$action = $direction == "INCOMING" ? "Deposited" : "Withdrawn";
		$branchArray = getRespectiveBranch($branchId,$conn);
		if($branchArray != 0){
			foreach($branchArray AS $branch){
				$branchFundsAccount = $branch['fundsAccount'];
			...
			$fundsFormatted = asMoney($amount);
			logUserActivity("$action funds worth $fundsFormatted/- to branch account: $branchFundsAccount","high",$conn);
			$branchBalance = getFundsBalance($conn,$branchId);
			switch($direction){
				case 'INCOMING':
				sendFundsDepositNotification($conn,$profileId,$amount,$branchBalance,$phoneNumber);
				break;

				case 'OUTGOING':
				// Manager SMS
				$fundsCategory = ucfirst($category);
				sendManagerDisbursementNotification($conn,$profileId,$amount,$receiptNumber,$charge,$branchBalance,$branchPhone,$fundsCategory);
				// Withdraw SMS
				sendFundsWithdrawNotification($conn,$profileId,$amount,$charge,$branchBalance,$phoneNumber);
				break;
			}
			$status = 1000;
		}
...
function sendFundsDepositNotification($conn,$profileId,$depositAmount,$branchBalance,$phoneNumber){
	$amountFormatted  = asMoney($depositAmount);
	$balanceFormatted = asMoney($branchBalance);
	$userArray        = getAccountHolder($conn,$profileId);
	if($userArray !=0 ){
		foreach($userArray AS $user){
			$profileId    = $user['id'];
		...
		if($branchArray != 0){
			foreach($branchArray AS $branch){
				$branchName    = strtoupper($branch['name']);
				$branchAccount = strtoupper($branch['fundsAccount']);
			}
		}
		$message = "$branchName has received your deposit of $amountFormatted/- to account number $branchAccount.Thank you!";
		$alertType = '43';
		sendNotification($message,$phoneNumber,$conn,$alertType,$profileId);
	}
}

function sendFundsWithdrawNotification($conn,$profileId,$amount,$charge,$branchBalance,$phoneNumber){
	$amountFormatted  = asMoney($amount);
	$balanceFormatted = asMoney($branchBalance);
	$chargeFormatted  = asMoney($charge);
	$userArray        = getAccountHolder($conn,$profileId);
	if($userArray !=0 ){
		foreach($userArray AS $user){
			$profileId    = $user['id'];
		...
		$message   = "Your float withdrawal of $amountFormatted/- from account $branchAccount is completed. \nNew Bal=$balanceFormatted/-.";
		$alertType = '44';
		sendNotification($message,$phoneNumber,$conn,$alertType,$profileId);
	}
}

function sendManagerDisbursementNotification($conn,$profileId,$amount,$receiptNumber,$charge,$branchBalance,$branchPhone,$fundsCategory){
	$amountFormatted  = asMoney($amount);
	$balanceFormatted = asMoney($branchBalance);
	$chargeFormatted  = asMoney($charge);
	$userArray        = getAccountHolder($conn,$profileId);
	if($userArray !=0 ){
		foreach($userArray AS $user){
			$profileId      = $user['id'];
			$clientFullName = strtoupper(strtolower($user['firstName'])).' '.strtoupper(strtolower($user['lastName']));
		}
		$message   = "$fundsCategory Disbursement of $amountFormatted/- to $clientFullName through $receiptNumber is completed.\nFloat Bal=$balanceFormatted/-.";
		$alertType = '45';
		sendNotification($message,$branchPhone,$conn,$alertType,$profileId);
	}
}

function voidFundsTransaction($conn,$transactionId){
	$voidQuery = "UPDATE fundsTransactions SET isVoid='1' WHERE id=$transactionId";
	return $conn->query($voidQuery) ? 1 : 0;
}

function createAirtimeTransactions($conn,$branchId,$profileId,$amount,$standard,$premium,$direction,$category,$description,$receiptNumber){
	$dateCreatedAt = date('Y-m-d H:i:s');
	$airtimeQuery = "INSERT INTO airtimeTransactions (branchId,profileId,amount,premium,standard,direction,category,description,receiptNumber,createdAt)
	VALUES('$branchId','$profileId','$amount','$premium','$standard','$direction','$category','$description','$receiptNumber','$dateCreatedAt')";
	if($conn->query($airtimeQuery)){
		/**
		 * LOG ACTION
		 * 
		 * SEND SMS
		...
		$branchBalance    = getAirtimeBalance($conn,$branchId);
		$airtimeFormatted = asMoney($amount);
		logUserActivity("Deposited branch airtime worth $airtimeFormatted/- to branch account: $branchAirtimeAccount",'high',$conn);
		if($direction == "INCOMING"){
			sendAirtimeDepositNotification($conn,$profileId,$amount,$branchBalance,$phoneNumber);
		}
		$status = 1000;
	}else{
		$status = 1001;
...
function sendAirtimeDepositNotification($conn,$profileId,$depositAmount,$branchBalance,$phoneNumber){
	$amountFormatted  = asMoney($depositAmount);
	$balanceFormatted = asMoney($branchBalance);
	$userArray        = getAccountHolder($conn,$profileId);
	if($userArray !=0 ){
		...
		if($branchArray != 0){
			foreach($branchArray AS $branch){
				$branchName    = strtoupper($branch['name']);
				$branchAccount = strtoupper($branch['airtimeAccount']);
			}
		}
		$message   = "Your airtime deposit of $amountFormatted/- to $branchName account $branchAccount is received.\nNew Bal=$balanceFormatted/-.";
		$alertType = '46';
		sendNotification($message,$phoneNumber,$conn,$alertType,$profileId);
	}
}

function voidAirtimeTransaction($conn,$transactionId){
	$voidQuery = "UPDATE airtimeTransactions SET isVoid='1' WHERE id=$transactionId";
	return $conn->query($voidQuery) ? 1 : 0;
}

function isFundsBalanceSufficient($conn,$branchId,$amount,$inclusive){
	$charges     = getCharge($conn,$amount);
	if($charges != 0){
		foreach($charges AS $charge){
			$chargeAmount = $charge['premium'];
		}
	}else{
		$chargeAmount = 0;
	}
	$total         = $inclusive ? ($amount + $chargeAmount) : $amount;
	$branchBalance = getFundsBalance($conn,$branchId);
	$balanceAfter  = $branchBalance - $total;
	return $branchBalance < $total || $balanceAfter < 0 ? 1001 : 1000;
}
/***
 * 
 * TRANSACTION CHARGES
 * 
 */
function getCharge($conn,$amount){
	$chargeQuery = "SELECT * FROM charges WHERE $amount BETWEEN minimum AND maximum";
	$charge = $conn->query($chargeQuery);
	if(!empty($charge)){
		$chargeArray = [];
 ...
 * 
 */
function createServiceFee($conn,$branchId,$fundsAccount,$type,$amount){
	$dateCreatedAt = date('Y-m-d H:i:s');
	$serviceQuery  = "INSERT INTO serviceFees (branchId,fundsAccount,type,amount,createdAt) VALUES('$branchId','$fundsAccount','$type',$amount,'$dateCreatedAt')";
	return $conn->query($serviceQuery) ? 1000 : 1001;
}